...
get_plrs_result<-function(x){#x表示一个向量
	temp_index<-x#
...
##2）.计算因子对基因表达的贡献度 
	...
	gene_id<-unique(big_table[temp_index,1])
	my_index<-as.numeric(big_table[temp_index,3])
	cnv_mut_M_random_temp<-cnv_mut_M_random[my_index]
	C_N_C_P_temp<-lapply(cnv_mut_M_random_temp,function(y){
		plsr_c_p(gene_id=gene_id,cnv_mutation_profile=y,gene_factor_list=gene_factor_list2)
		});
	save_file<-paste0("/pub5/xiaoyun/Jobs/J19/GBM_revise/gene_result/random_result/",temp_index[1],".RData");
