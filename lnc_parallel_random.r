...
get_plrs_result<-function(x){
	...
	##1）.依据所输入的候选driver gene序号将样本分成变异样本和野生型样本
		...
		plsr_c_p<-function(gene_id,cnv_mutation_profile,gene_factor_list){
		  ...
		  ## calculate the coeffients and pvalue of factors for all gene in case and contol samples  
		...
		lnc_id<-unique(big_table[temp_index,1])
		my_index<-as.numeric(big_table[temp_index,3])
		cnv_M_lnc_random_temp<-cnv_M_lnc_random[my_index]
		C_N_C_P_temp <- lapply(cnv_M_lnc_random_temp,function(y){
			plsr_c_p(gene_id=lnc_id,cnv_mutation_profile=y,gene_factor_list=gene_factor_list2)
		})
		save(C_N_C_P_temp, file=paste0("/pub5/xiaoyun/Jobs/J19/GBM_revise/lnc_result/random_result/",temp_index[1],".RData"));
