...
gene_diff_gene<-lapply(rownames(cnv_mut_M),function(x){names(gene_factor_list2)})
names(gene_diff_gene)<-rownames(cnv_mut_M)

##如果单基因内fdr校正，随机下游基因可以挑选
# load("gene_diff_relation_list_new.RData")
# gene_diff_gene<-sapply(gene_diff_relation_list_new,function(x){unique(x[,1])})

###构建随机的基因组变异谱（针对候选的driver基因）
library(BiRewire)
cnv_mut_M_random<-lapply(1:1000,function(x){
birewire.rewire.bipartite(cnv_mut_M,verbose=FALSE)
})
setwd(path1)
save(cnv_mut_M_random,file="cnv_mut_M_random.RData")

###构建并行所需要的索引表
big_table<-cbind(cbind(rep(names(gene_diff_gene),each=1000),rep(1:length(gene_diff_gene),each=1000)),rep(1:1000,length(gene_diff_gene)))
setwd(path1)
save(big_table,file="big_table.RData")

##一共11个数据，都不能少，注意加上gene_diff_gene,cnv_mut_M_random，gene_factor_list2
save(cnv_mut_M,copy_M,gene_M,gene_mir_list,gene_tf_list,meth_M,mir_M,tf_M,gene_factor_list2,gene_diff_gene,cnv_mut_M_random,file="random_data_required1.RData" )
###############################################################


##2. 随机并行开始
###########################################################
source(paste0(path0,"/gene_parallel_random.r"))
load(paste0(path1,"/big_table.RData"))
RSA(big_table);
RSA(get_plrs_result);
...
RSA(interIndex) ;
file.RData<-paste0(path1,"random_data_required1.RData")
RSA(file.RData) ;
RSE(IPP(interIndex,get_plrs_result,fire="weak", file.RData=file.RData, numThred.weak=8))
##############################################################


####3. 获取并行随机并进行识别显著差异调控边
###############################################################
load(paste0(path1,"/gene_diff_relation_list_new1.RData"))
setwd(paste0(path1,"/random_result"))
my_file<-dir()
...
get_downstream_effectors<-function(k){##i表示真实gene下标
	...
	for(my_file in zy_file){
		file_name_temp<-paste0(path1,"/random_result/",my_file)
		load(file_name_temp)
	...
	###计算差异的显著性
	diffK_score_list_temp<-gene_diffK_score_list[[k]]
	...
	save.file<-paste0(path1,"/temp/",k,"aa.RData")
	save(downstream_effectors,file=save.file)
	downstream_effectors
}

gene_downstream_effectors<-sapply(1:length(gene_diffK_score_list),get_downstream_effectors)
names(gene_downstream_effectors)<-names(gene_diffK_score_list)
setwd(path1)
save(gene_downstream_effectors,file="gene_downstream_effectors.RData")


gene_downstream_effectors_new<-sapply(gene_downstream_effectors,function(x){
# x=gene_downstream_effectors[[1]]
...
setwd(path1)
save(gene_downstream_effectors_new,file="gene_downstream_effectors_new.RData")


###4 导入ppi网络(使用全网络不是过滤之后的),保留在两步邻居内的下游基因
library(igraph)
load(main_path,cancer,"result/PPI_string.Rdata")
PPI<-as.data.frame(PPI,stringsAsFactors=F)
PPI[,3]<-as.numeric(PPI[,3])/1000
g<-graph.data.frame(PPI[,1:2],directed=FALSE)
g<-set.graph.attribute(g, "weight", PPI[,3])

a<-intersect(names(gene_downstream_effectors_new),V(g)$name)
index<-match(a,names(gene_downstream_effectors_new))
gene_downstream_effectors_new2<-gene_downstream_effectors_new[index]
response_genes_diffK<-lapply(names(gene_downstream_effectors_new2),function(temp_gene){
	index<-which(temp_gene==V(g)$name)
	ppi_gene<-as.character(neighborhood(g, order=2, nodes=V(g)$name[index])[[1]])
	pos<-which(temp_gene==names(gene_downstream_effectors_new2))
	downstream_gene<-gene_downstream_effectors_new2[[pos]]
	intersect(ppi_gene,downstream_gene)
})
names(response_genes_diffK)<-names(gene_downstream_effectors_new2)
path3<-paste0(main_path,cancer,"/analysis")
setwd(path3)
save(response_genes_diffK,file="response_genes_diffK.RData")